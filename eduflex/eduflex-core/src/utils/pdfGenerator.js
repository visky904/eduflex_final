...
// ---- PURE HTML BUILDER (Same content as generatePDF but no window, no printing) ----
function buildReportHTML(report) {
    ...
    // ---- REUSE EXACT SAME ACTIVITIES RENDER LOGIC BELOW ----
    html += generateActivitySection(report);
...
// ---- Extracted condition block (same as generatePDF but returns string) ----
function generateActivitySection(report) {
    if (report.activityType === 'MCQ') {
        return `
            ...
            <table>
                <thead><tr><th>Answer</th><th>Count</th><th>%</th></tr></thead>
                <tbody>
                    ${Object.entries(report.analysis.answerDistribution).map(([ans, count]) => `
                        <tr>
                            <td>${ans}</td><td>${count}</td><td>${((count / report.analysis.totalResponses) * 100).toFixed(1)}%</td>
                        </tr>`).join('')}
                </tbody>
            </table>
        `;
    ...
    if (report.activityType === 'Wordcloud') {
        return `
            <h2>☁️ Wordcloud Analysis</h2>
            <div class="stats-highlight"><strong>Total Words:</strong> ${report.analysis.totalWords}</div>
            ${report.analysis.allResponses.map(r => `<div class="response-box"><strong>${r.name}:</strong> ${r.text}</div>`).join('')}
        `;
    }

    if (report.activityType === 'Q&A') {
        return `
            <h2>💬 Q&A</h2>
            ${report.analysis.responses.map(r => `
                <div class="response-box"><strong>${r.name}</strong><p>${r.answer}</p></div>`).join('')}
        `;
    }

    if (report.activityType === 'Reviews') {
        return `
            <h2>⭐ Reviews</h2>
            <div class="stats-highlight"><strong>Avg Rating:</strong> ${report.analysis.averageRating}/5</div>
            ${report.analysis.individualRatings.map(r => `<div class="response-box"><strong>${r.name}:</strong> ${'⭐'.repeat(r.rating)}</div>`).join('')}
        `;
    }

    if (report.activityType === 'Feedback') {
        return `
            <h2>💭 Feedback</h2>
            ${report.analysis.feedback.map(f => `<div class="response-box"><strong>${f.name}:</strong> ${f.text}</div>`).join('')}
        `;
    }
    ...
    win.document.write(`
        ...
        <head>
            ...
            <style>
                ...
                /* Make combined report visually identical to individual report */
                .info-grid {
                    display: grid;
                    grid-template-columns: repeat(2, 1fr);
                    gap: 15px;
