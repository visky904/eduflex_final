...
export const generateSessionReport = (activity, responses, topic, roomCode) => {
    // --- PLAYLIST HANDLING ---
    if (activity.type === 'playlist' && activity.queue && Array.isArray(activity.queue)) {
        // Return an ARRAY of reports, one for each activity in the queue
        return activity.queue.map(item => {
            // Filter responses for this specific activity
            // Match activityId OR if it's a generic fallback (though for playlists we expect precise IDs)
            const itemResponses = responses.filter(r => r.activityId === item.playlistId && r.type === item.type);

            // Recursively generate report for this item
            return generateSessionReport(item, itemResponses, topic, roomCode);
        });
    }
    ...
    switch (activity.type) {
        ...
        case 'wordcloud':
            ...
            report.analysis = {
                question: activity.question || "Word Cloud",
                totalWords: Object.values(wordMap).reduce((a, b) => a + b, 0),
